...
class InsertDateByText extends InsertDateBase {
    ...
    constructor(props) {
        ...
        this.dayRule = [{ name: dataType.rules.NUMERIC, meta: { error: DAY_ERROR_MESSAGE, min: 1, max: 31 } }];

        this.monthRule = [{ name: dataType.rules.NUMERIC, meta: { error: MONTH_ERROR_MESSAGE, min: 1, max: 12 } }];

        this.yearRule = [{ name: dataType.rules.NUMERIC, meta: { error: YEAR_ERROR_MESSAGE, min: 1000, max: 9999 } }];
    }
